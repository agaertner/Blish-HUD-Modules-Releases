...
namespace Nekres.Screenshot_Manager
{
    internal class BrowserUtil
    {
        ...
        public static async void OpenInDefaultBrowser(string url)
        {
            ...
            {
                ScreenshotManagerModule.Logger.Warn($"Failed to run '{exe}' with arguments '{argString}': {e.Message}");
                Process.Start(url);
                return;
            }
        ...
        private static async Task<string> TryFetchWebPageTitle(string url)
        {
            ...
            return await request.GetResponseAsync().ContinueWith(async task => {
                if (task.IsFaulted) return string.Empty;
